            ...
            @foreach($libros as $libro)
                <div class="col">
                    <div class="card book-card">
                        <div class="book-cover text-center p-3">
                            <i class="fas fa-book fa-3x text-muted mb-2"></i>
                            <small class="d-block text-muted">{{ $libro->getTamanioFormateado() }}</small>
                        </div>
                        <div class="card-body text-center">
                            <h6 class="book-title">{{ $libro->titulo }}</h6>
                            <small class="text-muted">{{ $libro->autor }}</small>
                            @if($libro->anio_publicacion)
                                <br><small class="text-muted">{{ $libro->anio_publicacion }}</small>
                            @endif
                        </div>
